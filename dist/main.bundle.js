...
var CustomerCreateProfileComponent = (function () {
    function CustomerCreateProfileComponent(http, dataService, router, route) {
        ...
        this.disableCustomerInputs = false;
    ...
    CustomerCreateProfileComponent.prototype.validate = function () {
        ...
        if (this.customer.grpType == null) {
            errorMessageList.push('GroupType');
        }
        if (this.customer.grpCategory == null) {
            errorMessageList.push('GroupCategory');
        }
        if (this.customer.customerName != null && this.customer.groupName &&
            ...
            this.customer.grpType != null && this.customer.grpCategory != null &&
    ...
    CustomerCreateProfileComponent.prototype.setAccountTableList = function (accountList, listOfAcc, roleSelected) {
        ...
        if (this.invalidaccounts.length != 0) {
            this.invalidaccounts = this.invalidaccounts.concat(accountList.invalidaccountDetails);
        }
        else {
    ...
    /*Risk Monitoring*/
    CustomerCreateProfileComponent.prototype.getRecordFromTable = function (current, type) {
        current.selected = true;
        if (type == "add") {
            if (this.tempRiskMonitored.indexOf(current) === -1) {
                this.tempRiskMonitored.push(current);
            }
        }
        else if (type == "remove") {
            if (this.tempRiskRemoveMonitored.indexOf(current) === -1) {
                this.tempRiskRemoveMonitored.push(current);
            }
        }
    };
    /*Add,addall,remove,removeall functinality for table records*/
    CustomerCreateProfileComponent.prototype.add = function (type, records) {
        /*delete from parent*/
        var tempArray = [];
        for (var _i = 0, records_1 = records; _i < records_1.length; _i++) {
            var eachObj = records_1[_i];
            if (!eachObj.selected) {
        ...
        //Assiging
        if (type == "monitoring") {
            this.riskNotMonitoredList = this.riskNotMonitoredList.concat(this.copyrecords);
            this.monitoringRules = records;
        }
        else if (type == "servicetype") {
            this.notServices = this.notServices.concat(this.copyrecords);
            this.services = records;
            for (var i = 0; i < this.copyrecords.length; i++) {
                if (this.copyrecords[i].serviceType == '2nd Day Air') {
                    this.disableCustomerInputs = false;
                }
            }
        }
        else if (type == "daylist") {
            this.daysList = records;
            this.notDaysList = this.notDaysList.concat(this.copyrecords);
        }
        else if (type == "accessorials") {
            this.notAccessorials = this.notAccessorials.concat(this.copyrecords);
            this.accessorials = records;
        }
        else if (type == "riskException") {
            this.exceptionMonitors = this.exceptionMonitors.concat(this.copyrecords);
            this.notExceptionsMonitorlist = records;
        }
        else if (type == "shipmentCharge") {
            this.notShipmentCharges = this.notShipmentCharges.concat(this.copyrecords);
            this.shipmentCharges = records;
        }
        this.tempRiskMonitored = [];
        this.copyrecords = [];
    };
    CustomerCreateProfileComponent.prototype.addAll = function (toPush, currentRecords) {
        if (toPush == "monitoring") {
            ...
            for (var i = 0; i < currentRecords.length; i++) {
                if (currentRecords[i].serviceType == '2nd Day Air') {
                    this.disableCustomerInputs = false;
                }
            }
        }
    ...
    CustomerCreateProfileComponent.prototype.submit = function () {
        ...
        for (var i = 0; i < this.notServices.length; i++) {
            commit_end_day += ("E" + this.notServices[i].value + this.notServices[i].eHour + this.notServices[i].eMin + "C" + this.notServices[i].value + this.notServices[i].cHour + this.notServices[i].cMin);
        }
        /* Days list into single value*/
        var selectedDaysLength = this.notDaysList.length;
        var mondayText = "N";
        var tuesdayText = "N";
        var wednesdayText = "N";
        var thursdayText = "N";
        ...
        var json_to_send = {
            ...
            "mnrcnd": this.bindExceptionListToString(this.monitoringRules),
            "userId": "USER",
            "userName": "USERPD",
            "sites": "",
            "comment": ""
    ...
    CustomerCreateProfileComponent.prototype.resetForm = function () {
        location.reload();
    };
...
var CustomerProfileComponent = (function () {
    ...
    CustomerProfileComponent.prototype.ngOnInit = function () {
        ...
        this.dataService.getCustomers('http://localhost:3012/UPS/customersprofile')
            ...
            _this.setCustomersList(customersProfiles);
        });
    };
    CustomerProfileComponent.prototype.setCustomersList = function (customers) {
